...
void track_agents(struct RoboAI *ai, struct blob *blobs) {
  ...
  if (p) {
    ...
    mg = sqrt((vx * vx) + (vy * vy));
    if (mg > NOISE_VAR) {
      // Update heading vector if meaningful motion detected
      vx /= mg;
      vy /= mg;
      ai->st.bmx = vx;
      ai->st.bmy = vy;
...
void id_bot(struct RoboAI *ai, struct blob *blobs) {
  ...
  if (stepID >= 1 && ai->st.selfID == 1) {
    // Stop after a suitable number of frames.
    ai->st.state += 1;
    stepID = 0;
    BT_all_stop(0);
  ...
  **************************************************************************/
  ...
  if (ai->st.state == 0 || ai->st.state == 100 || ai->st.state == 200) {
    ...
    if ((ai->st.state % 100) != 0) {
      // The id_bot() routine will change the AI state to initial state + 1
    ...
    *****************************************************************************/
    ...
    // penalty kick mode
      ...
      fprintf(stderr, "penalty kick not implemented\n");
      exit(1);
      ...
      if (ai->st.self != NULL && ai->st.ball != NULL) {
        ...
        // if(!valid_location(tar_px, tar_py)){   // tar_px = 125;
        // tar_py = 384;
        // }
    ...
    // chase ball mode
      ...
      if (ai->st.self != NULL && ai->st.ball != NULL) {
        if(known_angle < 0 && calibrate_angle != 1) {
          calibrate_angle = 1;
          count_calibrate = 0;
        }
        ...
        double vel_ang = vector_angle(x, y);
        double mag = sqrt(pow(x, 2) + pow(y, 2));
        printf("Velocity: (%.2f, %.2f) -> %.2f -> %.2f\n", x, y, mag, vel_ang);

        for(int i = 0; i < (MODEL_SIZE - 1); i++) {
          velocity_mag[i] = velocity_mag[i+1];
          velocity_ang[i] = velocity_ang[i+1];
        }

        velocity_mag[MODEL_SIZE - 1] = mag;
        velocity_ang[MODEL_SIZE - 1] = vel_ang;
        ...
        if(calibrate_angle == 1) {
          BT_drive(MOTOR_A, MOTOR_B, 30);

          if(mag >= 15) {
            count_calibrate = count_calibrate + 1;
            if(count_calibrate >= 3) {
              known_angle = vel_ang;
              printf("Readings: %.2f\n", known_angle);
              calibrate_angle = 0;
              BT_drive(MOTOR_A, MOTOR_B, 0);
            }
          }
        ...
        int is_moving = 0;
        if(model_count >= MODEL_SIZE) {
          int half_model = MODEL_SIZE / 2;
          int model_val = 0;

          const double thresh = 15;
          for(int i = 0; i < MODEL_SIZE; i++) {
            if(velocity_mag[i] >= thresh) {
              model_val = model_val + 1;
            }
          }

          if(model_val >= half_model) {
            is_moving = 1;
          }
        }
        ...
        double hx = ai->st.sdx;
        double hy = ai->st.sdy;
        double ha_angle = vector_angle(hx, hy);
        double hb_angle = fmod(ha_angle + 180.0, 360.0);

        double dist_ha = fabs(known_angle - ha_angle);
        double dist_hb = fabs(known_angle - hb_angle);

        if(dist_hb < dist_ha) {
          known_angle = hb_angle;
        } else {
          known_angle = ha_angle;
        }

        if(is_moving == 1) {
          known_angle = velocity_ang[MODEL_SIZE - 1];
        }

        double self_px = ai->st.old_scx;
        double self_py = ai->st.old_scy;

        double ball_px = ai->st.old_bcx;
        double ball_py = ai->st.old_bcy;

        double diff_x = ball_px - self_px;
        double diff_y = ball_py - self_py;

        double face_to = vector_angle(diff_x, diff_y);
        double angle_delta = find_rotation_angle(known_angle, face_to);

        printf("Angle Ball: %.2f | Angle Blob: %.2f | Delta: %.2f\n", face_to, known_angle, angle_delta);

        if(fabs(angle_delta) > 30.0) {
          if(angle_delta < 0) {
            turn_left(15);
            usleep(250 * 1000);
            BT_drive(MOTOR_B, MOTOR_A, 0);
          } else {
            turn_right(15);
            usleep(250 * 1000);
            BT_drive(MOTOR_B, MOTOR_A, 0);
        ...
        // fprintf(stderr, "p-bot: (%.2f, %.2f)\n", self_px, self_py);
        // fprintf(stderr, "h-bot: (%.2f, %.2f)\n", ai->st.sdx, ai->st.sdy);

        // double mag = sqrt(pow(ai->st.svx, 2) + pow(ai->st.svx, 2));
        // fprintf(stderr, "v-bot: (%.2f, %.2f) -> %.2f\n", ai->st.svx, ai->st.svy, mag);
        // // angle and distance to the ball
        // double c_bs_theta, dis_ball_self;
        // c_bs_theta = dottie(b_s_dx, b_s_dy, mdx, mdy);
        // // c_bs_theta = dottie(b_s_dx, b_s_dy, s_h_dx, s_h_dy);
        // // fprintf(stderr, "dot p-> %.2f, angle p-> %.2f\n",c_bs_theta, (acos(c_bs_theta)/3.1415926) * 180);
